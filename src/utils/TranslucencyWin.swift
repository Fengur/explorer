...
class TranslucencyWin:NSWindow, NSApplicationDelegate, NSWindowDelegate{
    let w:CGFloat = 1000//1000
    let h:CGFloat = 800//800
    ...
    override init(contentRect: NSRect, styleMask aStyle: Int, backing bufferingType: NSBackingStoreType, `defer` flag: Bool) {
        ...
        let view = LiveEditView(w,h)//BasicView(w,h)//OtherView(w,h)////// (w,h)////AdvanceView(w,h)//BasicView(w,h)StashView(frame.width,frame.height)/*Sets the mainview of the window*/

...
class TranslucencyWin:NSWindow, NSApplicationDelegate, NSWindowDelegate{
    ...
    override init(contentRect: NSRect, styleMask aStyle: Int, backing bufferingType: NSBackingStoreType, `defer` flag: Bool) {
        ...
        let view = AdvanceView(w,h)//BasicView//LiveEditView(w,h)////(w,h)//OtherView//// (w,h)////AdvanceView(w,h)//StashView(frame.width,frame.height)/*Sets the mainview of the window*/
